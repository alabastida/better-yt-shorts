...
import { hasVideoEnded, isVideoPlaying, restartShort } from "./lib/VideoState";
...
export function main() {
  ...
  if (hasVideoEnded()) {
    if(options.numberOfReplays === state.currentPlay){
      handleAutoplay(state, settings, features["autoplay"]);
      state.currentPlay = 0;
    }else{
      state.currentPlay = (state.currentPlay as number) + 1;
      restartShort();
    }
  }
